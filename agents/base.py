...
class AuditTrail:
    ...
    def __init__(self, secret_key: Optional[str] = None):
        ...
        if not resolved_key:
            raise SecurityException(
                ...
                "Set it before instantiating AuditTrail (e.g. export AUDIT_SECRET_KEY=$(openssl rand -hex 32))."
            )
        if len(resolved_key) < 16:
            raise SecurityException("AUDIT_SECRET_KEY must be at least 16 characters for HMAC-SHA256 security.")
    ...
    def verify_integrity(self) -> bool:
        """Verify the full HMAC-SHA256 chain: re-compute each signature and check linkage."""
        for i, entry in enumerate(self.logs):
            # Verify chain linkage
            prev = self.logs[i-1]["current_hash"] if i > 0 else "GENESIS_BLOCK_0000000000000000"
            if entry["prev_hash"] != prev:
                return False
            # Re-compute and verify the HMAC signature
            sign_string = (
            ...
            if not hmac.compare_digest(expected_sig, entry["current_hash"]):
                return False
        return True
...
def _get_global_audit() -> AuditTrail:
    """Lazily initialize the global AuditTrail, enforcing env-var presence on first use."""
